...
- 一个项目对应一个可核验的公开源码入口；入口可以是独立仓库、当前仓库根目录，也可以是 `showcase/<slug>/` 子目录快照；
- `repoUrl` 必须指向实际公开位置，源码入口包含 README 和足以理解、运行核心功能的源码；配置方式写入文档，真实环境文件和凭据不入库；
...
`showcase/` 是从本机个人项目整理并经过脱敏检查的公开源码快照。项目卡的源码按钮通过内容文件中的 `repoUrl` 直达对应子目录；项目 slug、内容文件、`showcase/<slug>/` 目录和 GitHub 链接必须一一对应。
