...
- **宠物档案**：记录宠物品种、年龄、过敏信息、服务偏好和最近回访结果，敏感信息仅用于演示。
- **预约排班**：按服务项目、门店和服务师安排时段，自动检查容量与冲突，支持改约和取消。
- **服务执行**：签到后进入服务中，记录洗护、美容、SPA 或驱虫项目和耗材，完成后生成消费明细。
- **会员回访**：按次卡余次、疫苗提醒和服务周期生成回访任务，完成后沉淀下一次预约建议。

## 数据与验收

预约、宠物档案、服务记录、次卡和回访任务通过订单号关联。验收可以从移动端预约开始，经过签到、服务完成、次卡扣减和回访完成，后台的门店负载会同步变化。
