...
description: 申请、审批、报销、付款与归档一体化费控系统，适合小团队建立可追踪的费用流程。
...
金额、员工和票据均为虚构演示数据；审批、报销、付款和归档记录保留完整操作时间线。

## 业务模块

- **费用规则**：按部门、费用科目和金额区间配置预算与审批人，提交时校验可用额度。
- **报销单据**：员工可补充发票说明和附件，财务按申请、票据、预算和付款状态分段处理。
- **审批与付款**：支持多级审批、退回修改、付款批次确认和异常标记，付款前必须完成复核。
- **月度归档**：锁定账期后生成费用汇总，保留申请版本、审批意见和付款流水用于审计。

## 数据与验收

费用申请、预算占用、审批节点、票据附件和付款批次均有独立编号。验收路径为“提交申请—补充票据—审批—付款确认—归档”，重复提交不会产生第二条付款记录。
