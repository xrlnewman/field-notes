...
- **开票申请**：销售选择客户、合同和收入项目，系统带出税率与金额并校验可开余额。
- **审核与开具**：财务按发票类型、抬头和附件审核，退回时记录原因；开具后锁定关键字段。
- **回款核销**：登记银行回款、匹配发票与合同，支持部分回款、多笔核销和差异标记。
- **账期归档**：主管锁定月度账期，生成开票额、回款额、未核销额和异常清单。

## 数据与验收

申请、发票、回款、核销和账期均有独立流水号，重复回款不会重复核销。验收可从移动端发起申请，经过审核、开具、回款登记，最后在后台完成核销和归档。
