...
- **服务与时段**：维护服务项目、时长、价格、门店营业时间和可预约容量，冲突时以后端锁定结果为准。
- **预约与排班**：顾客选择门店、服务人员和时段后生成预约；前台可以改约、取消、补录签到，服务人员从个人日历接单。
- **核销与回访**：签到后进入服务中，完成时记录服务结果与耗材，系统自动生成评价和复购提醒任务。

## 数据与验收

核心数据包括预约单、时段库存、服务记录、支付定金、评价和回访任务。写接口使用业务唯一键与 `Idempotency-Key`，列表接口统一分页，关键状态均能在时间线中回放；演示时可从新建预约一路推进到评价完成。
