...
- **工单接入**：按来源、产品和优先级创建工单，自动分派到队列并计算首次响应 SLA。
- **协同处理**：客服可以转派、合并、添加内部备注和引用知识库，所有回复按时间线保留。
- **升级与质检**：临近 SLA 的工单进入风险看板，主管可升级处理；解决后发送满意度评价并沉淀标签。

## 数据与验收

工单、消息、SLA 计时、知识库文章和满意度结果分表保存，关闭工单需要解决摘要。验收可从客户创建开始，完成分派、回复、升级、解决和评价，后台统计同步刷新解决率与超时数。
