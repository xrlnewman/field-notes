...
- **设备台账**：维护设备编码、区域、运行状态、最近巡检时间和责任班组。
- **计划派单**：按周期生成巡检计划，分派到巡检员，记录接受、到场、完成和复核节点。
- **告警与复盘**：异常读数进入告警队列，支持确认、转派、关闭和关联巡检记录；月度页面对比能耗趋势与节能措施。

## 数据与验收

设备、巡检任务、读数、告警和事件日志按租户与区域隔离，告警关闭前必须填写处理说明。验收可从计划创建走到现场记录、复核归档，再回到告警复盘查看完整链路。
