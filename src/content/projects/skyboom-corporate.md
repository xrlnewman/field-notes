...
- **内容管理**：维护产品、新闻、案例、企业介绍和首页模块，支持草稿、预览、发布和下线。
- **多语言站点**：中文与英文内容按同一内容编号管理，缺少翻译时显示待补字段，避免线上出现空白页面。
- **询盘线索**：访客从产品页或联系页提交询盘，后台按来源、产品和负责人分派，跟进记录可回放。
- **站点配置**：统一维护导航、页脚、联系方式、SEO 标题和轮播顺序，发布前提供变更摘要。

## 数据与验收

内容、语言版本、媒体资源、询盘和跟进任务通过内容编号关联。验收流程为“创建产品—上传媒体—配置多语言—发布官网—提交询盘—后台跟进—关闭线索”，前台与后台的状态变化实时对应。
