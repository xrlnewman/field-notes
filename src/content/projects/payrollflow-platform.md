...
- **周期与人员**：创建薪资周期，锁定在职人员、部门和计薪规则，避免中途名单变化。
- **考勤绩效**：导入出勤、加班、奖金和扣款数据，系统生成差异清单供主管逐笔确认。
- **核算与审批**：计算应发、应扣和实发金额，按部门汇总后进入多级审批，异常记录必须有说明。
- **发放与工资条**：生成发放批次和员工工资条，模拟成功、失败重试与归档结果。

## 数据与验收

薪资周期、员工明细、差异项、审批事件和发放批次通过周期编号关联。验收流程为“创建周期—导入数据—复核差异—审批—发放—归档”，已锁定周期禁止重复计算。
