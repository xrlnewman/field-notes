...
description: 面向同城配送团队的运单、车辆、异常与司机协同平台，提供移动端工作台、运营后台与 Go Gin API。
...
repositories:
  - name: fleetflow-miniapp
    role: frontend
    description: 司机与调度员移动工作台，覆盖扫码接单、送达确认、签收记录和异常处理。
    tech: [Vite, TypeScript, Responsive UI]
    url: https://github.com/xrlnewman/fleetflow-miniapp
  - name: fleetflow-admin
    role: admin
    description: 调度后台与 Go Gin API，覆盖建单、分配、配送、签收、异常处理、结算确认，并提供 MySQL 8.4 / Redis 8 Compose。
...
screenshots:
  ...
  - src: /images/projects/fleetflow-platform/shipments-admin.png
    ...
    title: 运单调度
    caption: 运单可按状态分配司机并推进到送达、签收，表格保留路线、货物、车辆和预计送达等高价值字段。
    viewport: desktop
    width: 1045
    height: 1224
...
FleetFlow 是一套同城配送调度底座，面向商超配送、餐饮供应链、社区团购和小型物流团队。它把“建单—调度—在途—签收—异常—结算”串成一条可复盘的工作流。
...
- 运单调度：创建运单、分配司机，推进配送—送达—签收状态并查看事件时间线。
- 线路与车辆：用地图式线路舞台展示在途车辆，司机列表呈现在线、休息和利用率。
- 异常中心：超时、地址确认、车辆温度等异常带优先级和处理落库。
- 对账结算：按周展示运单数、司机数、金额并确认待结算账单。
- 数据边界：所有姓名、车牌、订单号均为虚构演示数据，不连接真实物流业务。

## 技术实现与运行范围

源码公开、允许自部署。后台采用 Vite、Go 1.25、Gin；MySQL 8.4 持久化业务与事件，Redis 8 提供 `Idempotency-Key` 幂等结果和资源锁，列表接口统一分页。
