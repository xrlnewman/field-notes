...
description: 合同起草、审批、电子签署、履约提醒和归档工作台，让合同从草稿到履约都有时间线。
...
- **业务发起人**：创建合同草稿、补齐对方信息、上传附件并提交审批。
- **法务与负责人**：按金额、合同类型和部门配置审批链，支持退回补充与重新提交。
- **履约管理员**：维护签署状态、付款节点、到期提醒和归档目录，异常合同进入风险清单。

## 数据与验收

合同主表关联参与方、审批节点、附件版本、付款计划和事件日志；同一合同的修改会保留版本号，避免审批后内容被静默替换。验收路径为“新建草稿—提交审批—通过—登记签署—添加履约提醒—归档”，每一步都返回统一响应并留下操作人和时间。
