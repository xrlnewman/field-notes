...
- GitHub 仓库 owner 固定为 `xrlnewman`、visibility 固定为 public、默认分支沿用 `master`。
...
提交格式 `docs(public): 补充商城关联仓库与公开说明`。创建三个 public GitHub 仓库，添加 `github` 远端，执行 `git push -u github master`，随后匿名确认 README 和默认分支。
...
创建三个同名仓库，添加 `github` 远端并推送 master，匿名确认可访问和三仓互链。
...
对九个 URL 执行匿名 HTTP GET 或 GitHub connector `get_repo`，确认 visibility 为 public、默认分支为 master、README 可读。
